...
class Seq2SeqModel:
    ...
    @torch.no_grad()
    def ce_losses(self, data, args=None):

        self._move_model_to_device()

        data = self.load_and_cache_examples(data)
        ...
        with tqdm(train_dataloader) as tq:
            for batch in tq:
                ...
                tq.set_postfix({"losses": mean(losses)})

        return losses

    @torch.no_grad()
    def project_inference_method(self, data_to_predict):

        self._move_model_to_device()
        dataset_to_predict = self.load_and_cache_examples(data_to_predict, verbose=verbose)
        probs_list, losses = self.infer_paraphrase_classification(dataset_to_predict)

        return probs_list

    def infer_paraphrase_classification(self, train_dataset):
        """
        ...
        with tqdm(train_dataloader) as tq:
            for batch in tq:
                ...
                mask = inputs['decoder_input_ids'] == 1 # mask paddings
                mask[:, :-1] = mask[:, 1:].clone() # left shift masking
                # logits[mask] = float('-inf')
                dists = logits.softmax(dim=2).max(dim=2).values

                # dists = logits.max(dim=2).values.softmax(dim=1)
                dists[mask] = 1
                probs = dists.log().mean(dim=1).exp()
                probs_list.extend(probs.tolist())
                sum_prob += probs.sum().item()
                count_probs += len(probs)
                mean_loss = torch.tensor(losses).mean().item()
                tq.set_postfix({"mean": sum_prob/count_probs, "losses": mean_loss})
                # print(probs)
                # print(sum_prob)
                # print(count_probs)
        print(sum_prob / count_probs)


        return probs_list, losses
    ...
    def train(
        ...
        if args.wandb_project:
            wandb.init(project=args.wandb_project, name=args.wandb_experiment, entity='asafmaman', config={**asdict(args)}, **args.wandb_kwargs)
